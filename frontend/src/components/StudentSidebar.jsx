...
import { FaBook, FaVideo, FaQuestionCircle, FaChartLine, FaGraduationCap, FaSignOutAlt, FaHome } from 'react-icons/fa';
const StudentSidebar = () => {
    ...
    return (<div className="fixed left-0 top-0 h-full w-64 bg-gradient-to-b from-slate-900 via-indigo-900 to-slate-900 text-white shadow-2xl z-50">
      <div className="flex flex-col h-full">
        ...
        <nav className="flex-1 p-4 space-y-2 overflow-y-auto">
          {menuItems.map((item) => {
            ...
            return (<Link key={item.title} to={item.path} className={`group relative flex items-center gap-3 px-4 py-3 rounded-xl transition-all duration-300 ${active
                ...
                {active && (<div className="absolute right-2 w-2 h-2 bg-white rounded-full animate-pulse shadow-lg shadow-white/50"></div>)}
              </Link>);
        })}
        </nav>
        ...
        <div className="p-4 border-t border-slate-700/50 space-y-3">
          {user && (<div className="bg-slate-800/50 backdrop-blur-sm rounded-xl p-3 mb-2 border border-slate-700/50 hover:border-blue-500/50 transition-all duration-300">
              <p className="text-xs text-slate-400 mb-1">Logged in as</p>
              <p className="text-sm font-semibold text-white truncate">{user.name}</p>
            </div>)}
          <button onClick={handleLogout} className="w-full flex items-center gap-3 px-4 py-3 rounded-xl transition-all duration-300 text-slate-300 hover:bg-slate-800/50 hover:text-white hover:scale-[1.02] active:scale-95 border border-transparent hover:border-slate-700/50">
