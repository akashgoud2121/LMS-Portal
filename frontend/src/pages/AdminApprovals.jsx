    ...
    return (<AdminLayout>
      ...
      <div className="mb-6 bg-white border rounded-lg p-4">
        <div className="flex flex-wrap gap-2">
          <button onClick={() => setFilter('all')} className={`px-4 py-2 rounded-md text-sm font-medium transition-colors ${filter === 'all'
            ? 'bg-blue-600 text-white'
            : 'bg-gray-100 text-gray-700 hover:bg-gray-200'}`}>
            All ({pendingUsers.length})
          </button>
          <button onClick={() => setFilter('student')} className={`px-4 py-2 rounded-md text-sm font-medium transition-colors ${filter === 'student'
            ? 'bg-blue-600 text-white'
            : 'bg-gray-100 text-gray-700 hover:bg-gray-200'}`}>
            <FaUserGraduate className="inline mr-2"/>
            Students ({studentCount})
          </button>
          <button onClick={() => setFilter('instructor')} className={`px-4 py-2 rounded-md text-sm font-medium transition-colors ${filter === 'instructor'
            ? 'bg-blue-600 text-white'
            : 'bg-gray-100 text-gray-700 hover:bg-gray-200'}`}>
            <FaChalkboardTeacher className="inline mr-2"/>
            Instructors ({instructorCount})
          </button>
          <button onClick={() => setFilter('admin')} className={`px-4 py-2 rounded-md text-sm font-medium transition-colors ${filter === 'admin'
            ? 'bg-blue-600 text-white'
            : 'bg-gray-100 text-gray-700 hover:bg-gray-200'}`}>
      ...
      {/* Pending Users List */}
      {pendingUsers.length === 0 ? (<div className="bg-white border rounded-lg p-12 text-center">
          ...
          {pendingUsers.map((user) => (<div key={user.id} className="bg-white border rounded-lg p-5 hover:shadow-md transition-shadow">
              <div className="flex items-start space-x-4">
                <div className={`w-12 h-12 rounded-full flex items-center justify-center text-white flex-shrink-0 ${user.role === 'admin'
                    ? 'bg-red-500'
                    : user.role === 'instructor'
                        ? 'bg-green-500'
                        : 'bg-blue-500'}`}>
                  {user.role === 'admin' ? (<FaShieldAlt className="text-xl"/>) : (<FaUser className="text-xl"/>)}
                </div>
                <div className="flex-1">
                  <div className="flex items-center space-x-2 mb-1">
                    <h3 className="text-lg font-semibold text-gray-900">{user.name}</h3>
                    <span className={`px-2 py-0.5 text-xs font-medium rounded ${user.role === 'admin'
              ...
              <div className="mt-4 pt-4 border-t border-gray-200">
                <div className="mb-4">
                  ...
                  <textarea placeholder="Enter reason for rejection..." rows={2} className="w-full px-3 py-2 border border-gray-300 rounded-md focus:ring-2 focus:ring-red-500 focus:border-red-500 text-sm" value={rejectionReason[user.id] || ''} onChange={(e) => setRejectionReason({ ...rejectionReason, [user.id]: e.target.value })}/>
                </div>
                <div className="flex gap-3">
                  <button onClick={() => handleApprove(user.id)} disabled={processing === user.id} className="flex-1 bg-green-600 text-white px-4 py-2 rounded-md font-medium hover:bg-green-700 disabled:opacity-50 transition-colors flex items-center justify-center gap-2">
                    <FaCheckCircle />
                    {processing === user.id ? 'Processing...' : 'Approve'}
                  </button>
                  <button onClick={() => handleReject(user.id)} disabled={processing === user.id} className="flex-1 bg-red-600 text-white px-4 py-2 rounded-md font-medium hover:bg-red-700 disabled:opacity-50 transition-colors">
                    Reject
                  </button>
                </div>
              </div>
