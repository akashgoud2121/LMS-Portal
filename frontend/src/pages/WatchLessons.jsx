...
const WatchLessons = () => {
    ...
    const getCompletedLessonIds = (enrollment) => {
        return enrollment.completedLessons?.map(cl => typeof cl === 'string' ? cl : cl.lesson?.toString() || cl.lessonId?.toString()) || [];
    };
    ...
    return (<div className="flex min-h-screen bg-gray-50">
      ...
      <div className="flex-1 ml-64 p-8">
        <div className="max-w-7xl mx-auto">
          ...
          {enrollments.length === 0 ? (<div className="bg-white rounded-2xl shadow-sm p-12 text-center border border-gray-100">
              <div className="mb-6">
              ...
              {enrollments.map((enrollment) => {
                ...
                return (<div key={enrollment._id} className="bg-white rounded-2xl shadow-sm border border-gray-100 hover:shadow-lg transition-all duration-300 overflow-hidden">
                    <div className="p-6">
                      <div className="flex items-start gap-6">
                        {enrollment.course.thumbnail && (<div className="flex-shrink-0">
                            <img src={getImageUrl(enrollment.course.thumbnail)} alt={enrollment.course.title} className="w-32 h-24 object-cover rounded-xl" onError={(e) => {
                        ...
                        <div className="flex-1">
                          ...
                          <div className="mb-4">
                            ...
                            <div className="w-full bg-gray-200 rounded-full h-2 overflow-hidden">
                              <div className="bg-gradient-to-r from-blue-500 via-indigo-500 to-blue-600 h-2 rounded-full transition-all duration-500" style={{ width: `${enrollment.progress}%` }}></div>
                            </div>
                          </div>

                          {nextLesson && (<Link to={`/courses/${enrollment.course._id}/lesson/${nextLesson._id}`} className="inline-flex items-center gap-2 bg-gradient-to-r from-blue-600 to-indigo-600 text-white px-6 py-2 rounded-xl hover:from-blue-700 hover:to-indigo-700 transition-all duration-300 hover:scale-105 active:scale-95 shadow-md hover:shadow-lg font-medium">
                      ...
                      {enrollment.course.lessons && enrollment.course.lessons.length > 0 && (<div className="mt-6 pt-6 border-t border-gray-100">
                          ...
                          <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
                            ...
                            return (<Link key={lesson._id} to={`/courses/${enrollment.course._id}/lesson/${lesson._id}`} className={`flex items-center gap-3 p-3 rounded-lg border transition-all duration-200 hover:scale-[1.02] ${isCompleted
                                    ? 'bg-blue-50 border-blue-200 hover:bg-blue-100'
                                    : 'bg-gray-50 border-gray-200 hover:bg-gray-100'}`}>
                                  <div className={`flex-shrink-0 w-8 h-8 rounded-lg flex items-center justify-center ${isCompleted ? 'bg-blue-600' : 'bg-gray-300'}`}>
                                    {isCompleted ? (<FaCheckCircle className="text-white text-sm"/>) : (<FaPlay className="text-white text-xs"/>)}
                                  </div>
                                  <div className="flex-1 min-w-0">
                                    <p className={`text-sm font-medium truncate ${isCompleted ? 'text-blue-700' : 'text-gray-700'}`}>
                                      {lesson.order}. {lesson.title}
                                    </p>
                                    {lesson.duration > 0 && (<p className="text-xs text-gray-500 flex items-center gap-1 mt-1">
