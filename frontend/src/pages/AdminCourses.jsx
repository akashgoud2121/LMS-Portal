...
const AdminCourses = () => {
    ...
    return (<AdminLayout>
      ...
      {/* Courses Grid */}
      {courses.length === 0 ? (<div className="bg-white rounded-xl shadow-lg p-12 text-center">
          <FaBook className="text-6xl text-gray-400 mb-4 mx-auto"/>
          <p className="text-gray-500 text-lg">No courses found</p>
        </div>) : (<div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
          {courses.map((course) => (<div key={course.id} className="bg-white rounded-xl shadow-md overflow-hidden hover:shadow-xl transition-all duration-300 hover:scale-105 border-2 border-transparent hover:border-blue-500">
              {/* Course Thumbnail */}
              {course.thumbnail && (<div className="relative overflow-hidden h-48 bg-gray-200">
                  <img src={getImageUrl(course.thumbnail)} alt={course.title} className="w-full h-full object-cover hover:scale-110 transition-transform duration-700" onError={(e) => {
              ...
              <div className="p-6">
                <div className="flex items-start justify-between mb-3">
                  ...
                  <span className={`px-2 py-1 text-xs font-semibold rounded-full ${course.isPublished
                ...
                <button onClick={() => handleTogglePublish(course.id, course.isPublished)} className={`w-full py-2.5 rounded-lg font-semibold transition-all duration-200 hover:scale-105 ${course.isPublished
