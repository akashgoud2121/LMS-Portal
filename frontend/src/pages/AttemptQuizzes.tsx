...
const AttemptQuizzes: React.FC = () => {
  ...
  const fetchQuizzes = async () => {
    try {
      ...
      const courseIds = enrollmentsResponse.data.map((e: any) => e.course._id);
      ...
      const quizPromises = courseIds.map(async (courseId: string) => {
        try {
          ...
          return response.data.map((quiz: any) => ({
            ...quiz,
            course: enrollmentsResponse.data.find((e: any) => e.course._id === courseId)?.course
          }));
        } catch (error) {
          return [];
        }
      ...
      const attemptPromises = allQuizzes.map(async (quiz: Quiz) => {
        try {
          ...
          if (response.data && response.data.length > 0) {
            const latestAttempt = response.data[response.data.length - 1];
                  ...
                  >
                    <div className="p-6">
                      ...
                      <Link
                        ...
                        className={`block w-full text-center py-3 rounded-xl font-medium transition-all duration-300 hover:scale-105 active:scale-95 shadow-md hover:shadow-lg ${
                          hasAttempted
